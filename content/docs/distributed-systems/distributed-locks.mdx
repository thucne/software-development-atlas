...
A distributed lock is not a local mutex stretched across the network. It is an **ownership protocol** that lets participants coordinate exclusive work while messages can be delayed, processes can pause, and holders can disappear.

A production-grade design must answer:

- what shared coordination system decides the current owner;
- how ownership expires or is renewed;
- how a holder proves that it still owns the lock;
- how stale holders are prevented from mutating the protected resource;
- how unlock avoids deleting another holder's ownership;
- how contenders wait without creating a polling storm;
- whether a lock is actually the right primitive for the invariant.

The key mental model is:

```text
lock acquisition -> temporary ownership -> protected work
                                   \-> ownership can become stale
```

Correctness therefore depends on **leases, ownership identity, and fencing**, not just on "only one process entered this code block."
