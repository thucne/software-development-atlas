...
A service often needs one business operation to do two durable things: update its database and tell another system what happened. Writing the database and publishing to a broker as two independent operations creates a **dual-write gap**: either side can succeed while the other fails.

The transactional outbox pattern closes that atomicity gap by writing the business change and an **outbox row that records the publication intent in the same database transaction**. A separate relay later publishes committed outbox records to the broker.

The useful contract is:

1. If the business transaction rolls back, there is no committed event to publish.
2. If the business transaction commits, the publication intent is durable even if the process crashes immediately afterward.
3. Publishing still happens asynchronously and can happen more than once, so consumers must remain idempotent.
