...
At 2:00 AM, an intermittent network glitch drops a single TCP acknowledgement packet between your consumer and the message broker. The broker assumes the message was never processed and immediately schedules a redelivery. Seconds later, your payment consumer ingests the same event a second time, charging a customer's credit card twice and dispatching duplicate confirmation emails. During the subsequent post-mortem, an engineer protests: *"Why did this happen? Our message broker documentation claims guaranteed delivery!"*

Distributed systems do not operate on magic; they operate over unreliable networks. When network partitions, crashes, or timeouts occur, the fundamental mechanics of message handoff dictate what can and cannot be guaranteed.

## TL;DR

> 💡 **Rule of thumb:** Delivery guarantees belong to the network transport, but deduplication belongs to the business entity. Never rely on a message broker to guarantee "exactly-once" business side effects without an idempotent consumer.

- **At-most-once:** Prioritizes throughput and zero duplicate risk over durability. Messages are dispatched without retries; if a crash occurs during transit or handling, the message is permanently lost.
- **At-least-once:** The pragmatic production default for reliable messaging. The broker retransmits messages until an explicit acknowledgement (ACK) is received, meaning **duplicate deliveries are inevitable** under real-world network turbulence.
- **"Exactly-once" processing:** A strictly bounded abstraction (such as Apache Kafka's transactional stream processing), never a universal guarantee covering external databases, third-party payment APIs, or email providers.
- **Ordering is decoupled from delivery:** At-least-once retries frequently disrupt message arrival sequences; partition-level or key-level serialization and monotonic versioning must be handled explicitly.
- **Fatal pitfall:** Acknowledging an event before the database commit drops messages forever if the process crashes; acknowledging after the commit without an idempotent consumer or inbox table causes duplicate financial or operational side effects upon redelivery.
