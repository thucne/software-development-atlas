...
# Web Accessibility: Build Resilient Interfaces Across Input and Ability

## TL;DR

A financial services firm redesigns its web checkout with sleek, borderless custom dropdowns built from `<div>` tags and `onClick` handlers. The next month, the company receives a civil rights accessibility lawsuit: screen reader users cannot find or activate the checkout trigger, keyboard-only power users get trapped in an un-focusable modal loop, and form validation errors only highlight input borders in light red without text descriptions or live region announcements. A ten-minute manual audit with `Tab` and VoiceOver could have prevented months of legal liability.

> 💡 **Rule of thumb:** **Use semantic HTML first; ARIA is a supplement, not a substitute for native controls.** If a native `<button>`, `<input>`, or `<dialog>` element exists, use it. Native controls give you keyboard navigation (`Space`, `Enter`, arrows), accessible names, focus states, and screen reader announcements for free without custom ARIA emulation.

- **Semantic HTML baseline:** Native elements provide built-in focusability, keyboard events, and accessible roles without custom JavaScript or synthetic event handlers.
- **Accessible name & description:** Ensure every interactive control has an unambiguous accessible name via visible text, `<label for>`, or `aria-label`/`aria-labelledby`.
- **Keyboard operability & visible focus:** Ensure every user flow is fully operable using only the keyboard (`Tab`, `Shift+Tab`, `Enter`, `Escape`), with visible focus indicators and modal focus management.
- **Perceivable forms & live regions:** Associate error messages with inputs using `aria-describedby` and `aria-invalid`; announce dynamic async updates with `aria-live="polite"` or `role="status"`.
- **Fatal pitfall:** Building interactive controls out of non-semantic tags (`<div onClick={...}>`) without `tabIndex="0"`, keyboard listeners, or accessible labels—rendering critical features completely invisible and inoperable to assistive technology users.
