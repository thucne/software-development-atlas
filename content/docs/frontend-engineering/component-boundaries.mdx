...
An engineer splits a 600-line `CheckoutForm.tsx` file into 12 micro-components ('Header', 'InputWrapper', 'SubmitRow', 'Notice') because a linter flagged file length. Two weeks later, a minor change to discount code validation unexpectedly breaks shipping address calculation, requiring prop drilling through 8 levels of components and causing 14 re-renders on every keystroke. The root cause? Slicing by visual layout instead of **cohesive responsibility and change boundaries**, fragmenting state ownership across unrelated components.

> 💡 **Rule of thumb:** **Split components by reason to change and state ownership, never by line count or aesthetic file size.** Keep code together when parts share a single interaction invariant; draw an explicit boundary when sub-trees change for different reasons, need distinct data owners, or benefit from decoupled composition via `children` slots.

- **Cohesive responsibility over visual slicing:** A well-designed boundary encapsulates cohesive behavior behind a minimal, explicit contract of props and semantic intent callbacks.
- **State ownership clarity:** Every piece of mutable state must have a single authoritative owner; never scatter stateful invariants across artificial presentation boundaries.
- **Change locality:** When product requirements evolve (e.g. pricing, search filters), the change blast radius must stay strictly local to that component boundary rather than rippling across parent trees.
- **Composition over prop drilling:** Use component composition (`children`, render slots) to invert control and avoid passing irrelevant props through intermediate presentation layers.
- **Fatal pitfall:** Prematurely decomposing tightly coupled markup into dozens of trivial single-use components—multiplying interface boilerplate and prop drilling while destroying cohesion and change locality.
