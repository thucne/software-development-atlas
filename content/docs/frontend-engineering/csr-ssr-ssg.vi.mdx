...
CSR, SSR và SSG trả lời một câu hỏi nền tảng: **HTML hữu ích của một route được tạo ở đâu và khi nào?** Câu trả lời thay đổi first response, thời điểm lấy dữ liệu, khả năng reuse cache, server work, client work và failure mode.
...
Điều này giúp nội dung dùng chung rất dễ cache và có thể loại full page generation khỏi request path thông thường. Trade-off là độ mới: output đã tạo chỉ hợp lệ khi sản phẩm còn chấp nhận tuổi của nó hoặc khi publishing/revalidation refresh đủ nhanh.
