...
# Frontend Data Fetching: Make Remote Data Explicit

## TL;DR

Remote data is still **server-owned state** even when the browser keeps a cached copy. A reliable frontend therefore needs more than `fetch()` calls: it needs a stable request identity, an intentional fetch location, explicit freshness rules, race handling, mutation invalidation, and UI states for waiting and failure.

A useful review sequence is:

1. What remote fact are we reading, and who is authoritative?
2. What inputs define the request identity?
3. Should this request start at a server/route boundary or in the client?
4. Which requests are independent and can start in parallel?
5. Which cache layer owns reuse, freshness, and deduplication?
6. What prevents an old response from overwriting a newer view?
7. After a mutation, which observations are now stale?
