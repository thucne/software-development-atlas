---
title: "Khả năng truy cập Web: Vận hành giao diện ngoài luồng lý tưởng"
...
# Khả năng truy cập Web: Vận hành giao diện vượt qua phạm vi màn hình

## TL;DR

Một công ty dịch vụ tài chính thiết kế lại trang thanh toán bằng các dropdown tùy biến làm hoàn toàn bằng thẻ `<div>` và sự kiện `onClick`. Tháng sau, công ty đối mặt với đơn kiện vi phạm quyền tiếp cận thông tin: người dùng khiếm thị dùng trình đọc màn hình không thể tìm thấy nút thanh toán, người dùng điều hướng bằng bàn phím bị kẹt cứng trong modal không thể thoát ra, còn thông báo lỗi form chỉ đổi viền đỏ nhạt mà không hề có nhãn chữ hay thông báo động (live region). Chỉ mười phút kiểm thử thủ công bằng phím `Tab` và VoiceOver đã có thể ngăn chặn rủi ro pháp lý kéo dài nhiều tháng.

> 💡 **Quy tắc bỏ túi:** **Ưu tiên HTML ngữ nghĩa trước tiên; ARIA chỉ là phần bổ trợ, không thể thay thế control gốc.** Nếu phần tử HTML gốc (`<button>`, `<input>`, `<dialog>`) đã hỗ trợ, hãy dùng nó. Thẻ HTML gốc cung cấp miễn phí khả năng điều hướng bàn phím (`Space`, `Enter`, phím mũi tên), tên truy cập, quản lý focus và tương thích hoàn hảo với trình đọc màn hình mà không cần giả lập ARIA phức tạp.

- **Nền tảng HTML ngữ nghĩa:** Các phần tử gốc cung cấp sẵn khả năng focus, sự kiện bàn phím và vai trò trợ năng chuẩn hóa mà không cần viết thêm JavaScript hay sự kiện giả lập.
- **Tên và mô tả truy cập (Accessible Name & Description):** Đảm bảo mọi control tương tác đều có tên truy cập rõ ràng qua văn bản hiển thị, `<label for>`, hoặc thuộc tính `aria-label`/`aria-labelledby`.
- **Khả năng vận hành bằng bàn phím & hiển thị focus:** Đảm bảo toàn bộ luồng tác vụ có thể thực hiện chỉ bằng bàn phím (`Tab`, `Shift+Tab`, `Enter`, `Escape`), giữ chỉ báo focus rõ ràng và bẫy focus trong modal.
- **Biểu mẫu dễ cảm nhận & vùng live region:** Gắn kết thông báo lỗi với ô nhập liệu bằng `aria-describedby` và `aria-invalid`; thông báo các cập nhật dữ liệu động bằng `aria-live="polite"` hoặc `role="status"`.
- **Cạm bẫy chết người:** Xây dựng các nút bấm hay menu bằng thẻ phi ngữ nghĩa (`<div onClick={...}>`) mà không có `tabIndex="0"`, thiếu lắng nghe phím bấm và nhãn trợ năng—khiến tính năng hoàn toàn vô hình và tê liệt đối với người dùng công nghệ hỗ trợ.
...
**Hậu quả:** người dùng chuột trên luồng lý tưởng (happy path) vẫn checkout được, trong khi người dùng bàn phím tab ra sau modal, người dùng screen reader khó nhận diện/chọn suggestion, người dùng low vision bỏ lỡ lỗi chỉ báo bằng màu và async success state hoàn toàn im lặng.
...
- [ ] Đưa loading, empty, error và success state vào accessibility check, không chỉ ở luồng lý tưởng (happy path).
