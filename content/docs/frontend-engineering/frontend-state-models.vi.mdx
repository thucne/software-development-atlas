...
# Các mô hình Frontend State: Sở hữu, dẫn xuất hoặc lưu bền

## Tóm tắt

Một người dùng lọc danh mục phòng khách sạn theo tiêu chí "Giá < 1 triệu" và "Gần biển", tìm thấy phòng ưng ý và sao chép link trình duyệt gửi cho bạn. Khi người bạn mở link, trang web lại hiển thị danh sách phòng ngẫu nhiên không có bộ lọc. Cùng lúc đó tại tab gốc, một lập trình viên sao chép dữ liệu API từ server vào cả store toàn cục lẫn local state component, gây ra vòng lặp `useEffect` đồng bộ vô tận làm đóng băng trình duyệt. Nguyên nhân gốc rễ? Đặt nhầm trạng thái URL vào local state và tạo ra các bản sao có thể ghi cạnh tranh nhau từ dữ liệu thuộc sở hữu của máy chủ.

> 💡 **Quy tắc bỏ túi:** **Mỗi sự thật trạng thái chỉ được có một chủ sở hữu duy nhất—tất cả những thứ còn lại hãy tính toán dẫn xuất (derive).** Nếu trạng thái mang ý nghĩa điều hướng (bộ lọc, tab, từ khóa), hãy lưu vào URL để người dùng có thể chia sẻ và bookmark; nếu là dữ liệu từ API, hãy coi đó là cache tạm thời thuộc sở hữu của máy chủ; không bao giờ sao chép dữ liệu từ server hay props vào local state rồi dùng `useEffect` để đồng bộ thủ công.

- **Phân loại trạng thái theo 4 chiều:** Luôn phân loại thông tin theo: chủ sở hữu có thẩm quyền (local UI, URL, server cache, workflow dùng chung), thời gian sống, phạm vi ảnh hưởng và nhu cầu lưu bền.
- **URL là chủ sở hữu trạng thái:** Đưa các tham số tìm kiếm, view đang kích hoạt, bộ lọc và phân trang vào URL search params để lịch sử trình duyệt (back/forward) và chia sẻ link hoạt động hoàn hảo.
- **Một nguồn sự thật duy nhất & trạng thái dẫn xuất:** Tính toán giá trị trực tiếp trong quá trình render (ví dụ: `const visibleItems = items.filter(...)`) thay vì lưu bản sao phụ đòi hỏi phải cập nhật đồng bộ thủ công.
- **Dữ liệu server thuộc sở hữu của server:** Coi dữ liệu API là bản cache bất đồng bộ của máy chủ; dùng công cụ quản lý cache chuyên dụng (TanStack Query, SWR, RSC) thay vì nạp dữ liệu thô vào global store client.
- **Cạm bẫy chết người:** Sao chép dữ liệu server hoặc props vào local `useState` rồi cố gắng duy trì đồng bộ hai chiều bằng `useEffect`—chắc chắn sẽ gây ra giao diện lỗi thời, race condition và các vòng lặp re-render vô tận.
...
**Hậu quả:** Back/Forward khôi phục filter khác với URL đang hiển thị, shared link mở với default hoặc local preference cũ, reload phụ thuộc thứ tự effect, và engineer không xác định được layer nào authoritative khi có sự cố.
