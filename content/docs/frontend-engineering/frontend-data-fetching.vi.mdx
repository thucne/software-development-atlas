...
# Lấy dữ liệu Frontend: Định danh request, caching và race condition

## Tóm tắt

Một người dùng tìm kiếm vé máy bay đi "Tokyo", sau đó đổi ý và gõ ngay "Paris". Phản hồi cho "Paris" trả về nhanh chóng sau 150ms và hiển thị các chuyến bay đi Paris. Nhưng 800ms sau, phản hồi rùa bò cho "Tokyo" mới cập bến, âm thầm ghi đè toàn bộ màn hình thành vé đi Tokyo trong khi ô tìm kiếm vẫn ghi rõ "Paris"! Khách hàng đặt nhầm phòng khách sạn ở Tokyo, gây ra một đợt khiếu nại hỗn loạn. Nguyên nhân? Lỗi race condition kinh điển khi dùng `useEffect` gọi `fetch()` thô sơ mà không có định danh request, thiếu hủy request bằng tín hiệu abort và không có quản lý vòng đời cache.

> 💡 **Quy tắc bỏ túi:** **Dữ liệu từ xa thuộc sở hữu của máy chủ, không phải state cục bộ—hãy coi đó là một bản quan sát có lưu cache.** Mọi truy vấn cần một định danh request ổn định (cache key); mọi request đang bay phải hủy được qua `AbortController`; và các thao tác cập nhật (mutation) phải làm mất hiệu lực (invalidate) các query liên quan thay vì cầu may rằng client sẽ tự đồng bộ.

- **Dữ liệu server là trạng thái thuộc sở hữu của server:** Coi dữ liệu API là một bản chụp bất đồng bộ của máy chủ; quản lý bằng các công cụ cache chuyên dụng (TanStack Query, SWR, RSC) thay vì dùng `useEffect` + `useState` thủ công.
- **Định danh request (Request Identity) ổn định:** Xây dựng cache key chứa đầy đủ mọi tham số đầu vào (tham số truy vấn, bộ lọc, phân trang, user ID); các key giống nhau sẽ tự động gộp request và khử trùng lặp mạng.
- **Race condition & hủy bỏ yêu cầu:** Sử dụng tín hiệu `AbortController` để các lần tìm kiếm kế tiếp nhanh chóng hủy bỏ hoặc bỏ qua các phản hồi cũ trả về muộn từ truy vấn trước.
- **Tải song song thay vì thác nước (Waterfall):** Đẩy các yêu cầu dữ liệu độc lập lên ranh giới route hoặc Server Component, kích hoạt song song qua `Promise.all` thay vì gọi tuần tự lồng nhau bên trong các component con.
- **Làm mất hiệu lực (Invalidation) & cập nhật lạc quan (Optimistic Updates):** Khi thay đổi dữ liệu, hãy chủ động làm mất hiệu lực các query bị ảnh hưởng hoặc cập nhật giao diện trước với cơ chế tự động rollback nếu gặp lỗi mạng.
- **Cạm bẫy chết người:** Viết `useEffect(() => { fetch(...) }, [query])` trần trụi mà không có abort signal—cho phép các phản hồi mạng về sai thứ tự ghi đè lên giao diện người dùng mới hơn bằng dữ liệu cũ kỹ.
