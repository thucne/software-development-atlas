---
title: "Search Indexes: Suy luận về phân tích văn bản, độ liên quan và độ mới"
...
# Search Indexes: Suy luận về phân tích văn bản, độ liên quan và độ mới

## Tóm tắt

Một nhóm kỹ sư quyết định tinh giản kiến trúc bằng cách chọn luôn Elasticsearch làm cơ sở dữ liệu chính—tự tin biến nó thành **nguồn sự thật (source of truth)** duy nhất của toàn hệ thống. Vài tháng sau, một sự cố mất điện kèm lỗi phân mảnh ổ đĩa xảy ra; các giao dịch đang xử lý dở dang bốc hơi hoàn toàn do thiếu cơ chế write-ahead log với ràng buộc giao dịch nghiêm ngặt, dẫn tới mất mát dữ liệu vĩnh viễn không thể cứu vãn. Trước đó, bộ phận hỗ trợ khách hàng đã liên tục tiếp nhận khiếu nại: "Tôi vừa bấm tạo bài viết xong mà tìm kiếm ngay lại không thấy!". Đội ngũ phát triển đã vấp phải đặc tính cốt lõi của kiến trúc **gần thời gian thực (near-real-time - NRT)**: chu kỳ **refresh interval** mặc định 1 giây tạo ra một khoảng trễ (staleness) rõ rệt khiến tài liệu vừa ghi chưa thể hiển thị ngay trong kết quả tìm kiếm. Đỉnh điểm của thảm họa xảy ra khi hệ thống cho phép index tùy tiện các payload JSON lồng nhau từ người dùng mà không có schema kiểm soát, kích hoạt **thảm họa Mapping Explosion** sinh ra hàng chục nghìn trường mới, làm nghẽn heap bộ nhớ của master node và đánh sập hoàn toàn cụm tìm kiếm.

> 💡 **Quy tắc bỏ túi:** Hãy luôn coi search index là một hình chiếu phái sinh (derived projection) chuyên trách truy xuất từ khóa, xếp hạng và lọc dữ liệu—tuyệt đối không dùng làm **nguồn sự thật (source of truth)** lưu trữ giao dịch gốc. Luôn thiết lập mapping tường minh giữa **trường text** và **trường keyword**, làm chủ độ trễ làm mới **NRT (gần thời gian thực)**, và kiểm soát chặt chẽ cấu trúc schema để tránh thảm họa bùng nổ mapping.

- **Chỉ mục đảo (inverted index) đảo ngược chiều truy vấn:** Thay vì quét tuyến tính từng tài liệu, **inverted index (chỉ mục đảo)** map các từ khóa chuẩn hóa tới danh sách tài liệu tương ứng; **bộ phân tích (analyzer)** chịu trách nhiệm tách từ, chuẩn hóa và sinh ra chuỗi **token** quyết định độ khớp của truy vấn.
- **Phân định rõ ràng trường text và trường keyword:** Áp dụng **trường text (`text` field)** cho ngôn ngữ tự nhiên (truy vấn bằng **`match` query** và xếp hạng bằng thuật toán tương đồng **BM25**), và dùng **trường keyword (`keyword` field)** cho các mã định danh, trạng thái cần lọc chính xác, sắp xếp hoặc gom nhóm (aggregation).
- **Khả năng hiển thị là gần thời gian thực (near-real-time - NRT):** Dữ liệu ghi mới được đệm trên RAM và chỉ hiển thị trong kết quả tìm kiếm sau thao tác **refresh** tạo segment mới; đừng lạm dụng ép refresh đồng bộ trên từng thao tác ghi nếu không muốn bóp nghẹt thông lượng ghi.
- **Tiến hóa schema an toàn bằng reindex và alias:** Cấu trúc phân tích từ khóa một khi đã lưu vào segment thì không thể sửa đổi tại chỗ; việc nâng cấp analyzer hay đổi kiểu dữ liệu bắt buộc phải chạy qua quy trình **reindex** sang index mới và hoán đổi con trỏ **alias** một cách nguyên tử để ngăn chặn độ trễ (stale).
- **Cạm bẫy chết người (Ảo tưởng CSDL chính & thảm họa bùng nổ mapping):** Coi search index là database chính khiến hệ thống đối mặt với nguy cơ mất dữ liệu khi crash, trong khi việc để ứng dụng tự động sinh trường JSON không kiểm soát sẽ gây ra **thảm họa Mapping Explosion**, làm nghẽn heap bộ nhớ của master node và tê liệt toàn bộ cụm máy chủ.

Mental model:

```text
tài liệu gốc (source document)
  -> mapping quyết định ngữ nghĩa của từng trường dữ liệu
  -> analyzer chuyển đổi văn bản thành các token chuẩn hóa
  -> inverted index ánh xạ từng từ khóa (term) tới các tài liệu phù hợp
  -> chuỗi truy vấn được phân tích tương thích
  -> lọc và truy xuất các tài liệu ứng viên
  -> tính điểm tương đồng (relevance scoring) để xếp hạng kết quả
  -> cơ chế refresh quyết định khi nào dữ liệu mới ghi có thể tìm kiếm được
```
...
```text
relational index (chỉ mục quan hệ)
  giá trị cột -> đường dẫn truy cập có thứ tự tới các hàng dữ liệu gốc (authoritative rows)

search index (chỉ mục tìm kiếm)
  từ khóa đã phân tích + trường có cấu trúc -> danh sách ID tài liệu ứng viên + điểm số tương đồng
```

Thuật ngữ **index (chỉ mục)** xuất hiện trong cả hai hệ thống, nhưng mô hình dữ liệu và bài toán về tính đúng đắn lại hoàn toàn khác nhau.

Index trong cơ sở dữ liệu quan hệ được công cụ cơ sở dữ liệu duy trì một cách đồng bộ và đảm bảo tính giao dịch (ACID) cùng với bảng dữ liệu. Trong khi đó, một search engine độc lập thường được cập nhật bất đồng bộ từ nguồn sự thật khác, tạo ra một ranh giới rõ ràng về độ trễ dữ liệu (freshness boundary).

## 2. Inverted index đảo chiều lookup

<TermBox term="Inverted index">
**Inverted index (chỉ mục đảo)** là cấu trúc ánh xạ các từ khóa tìm kiếm (term) tới danh sách các tài liệu chứa từ khóa đó, thường đi kèm các siêu dữ liệu bổ trợ như tần suất xuất hiện (term frequency) hoặc vị trí của từ trong văn bản.

Thay vì phải duyệt tuần tự qua mọi tài liệu để hỏi “tài liệu này có chứa từ `database` không?”, công cụ tìm kiếm chỉ việc tra cứu từ khóa `database` rồi lập tức lấy về posting list các tài liệu phù hợp.

**Vì sao quan trọng:** Tốc độ vượt trội của tìm kiếm toàn văn bắt nguồn từ việc tính toán sẵn cấu trúc tra cứu ngược này. Tuy nhiên, những gì được đưa vào cấu trúc phụ thuộc hoàn toàn vào các quyết định phân tích từ khóa (**analyzer**) và thiết lập mapping ngay tại thời điểm lập chỉ mục.
</TermBox>

Giả sử ba document chứa nội dung sau:
...
Một inverted index đơn giản có dạng:
...
Các search engine thực tế lưu trữ siêu dữ liệu phong phú hơn nhiều, nhưng mô hình tư duy này đủ để giải thích vì sao một sai lầm trong khâu phân tích từ khóa có thể khiến tài liệu trở nên vô hình đối với truy vấn của người dùng.

## 3. Mapping quyết định một field có ý nghĩa gì

Trong Elasticsearch, mapping định nghĩa kiểu dữ liệu của trường (`field type`) và quyết định cách giá trị được lập chỉ mục cũng như cách truy vấn.

Hai kiểu trường dạng chuỗi xuất hiện liên tục:

### `text`

Một **trường text (`text` field)** được phân tích qua bộ analyzer để phục vụ tìm kiếm toàn văn (full-text search). Nội dung ngôn ngữ tự nhiên như tiêu đề sản phẩm hay bài viết sẽ được tách từ và chuẩn hóa để truy vấn có thể khớp theo từng từ riêng lẻ thay vì bắt buộc phải khớp chính xác 100% toàn bộ chuỗi ban đầu.

### `keyword`

Một **trường keyword (`keyword` field)** giữ nguyên vẹn giá trị chuỗi có cấu trúc để phục vụ các thao tác khớp chính xác tuyệt đối, chẳng hạn như bộ lọc (filter), sắp xếp (sort) và tổng hợp thống kê (aggregation).
...
status:      keyword
sku:         keyword
brand_name:  text + keyword (multi-field khi vừa cần tìm kiếm vừa cần gom nhóm chính xác)
```
...
**Analyzer (bộ phân tích)** chuyển text thành token khi indexing hoặc querying. Nó có thể tách từ (tokenize) rồi áp dụng chuẩn hóa như chuyển chữ thường, loại bỏ từ dừng (stop-words), đưa từ về dạng gốc (stemming) hoặc các bộ lọc token khác.
...
Balance đúng phụ thuộc product. Search legal clause, e-commerce product, source code, support ticket và observability log có đánh đổi rất khác nhau.
...
## 11. Segment giải thích sự đánh đổi giữa refresh và throughput
...
Đánh đổi quen thuộc từ bài Partitioning & Sharding:
