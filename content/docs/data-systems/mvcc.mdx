...
A transaction does not need to be actively querying every second to matter. An application that begins a transaction and then waits on user input, remote I/O, a job queue, or forgotten connection-pool code can hold a snapshot much longer than intended when its isolation/cursor semantics retain that snapshot.

Practical review questions include:

- How long can this transaction remain open?
- Does the isolation level retain one snapshot for the transaction?
...
## 13. Production scenario: a long-lived reporting snapshot causes table growth

A reporting endpoint starts a `REPEATABLE READ, READ ONLY` transaction so one export sees a stable database view. It runs an initial query, then performs several remote enrichment calls before committing. A connection timeout bug sometimes leaves the session `idle in transaction` for hours. Meanwhile, a high-write `events` table receives frequent updates and deletes.

**Impact:** dead tuples accumulate, table and index sizes grow, vacuum cannot reclaim as much space as expected, query latency degrades as more pages are touched, and maintenance pressure rises even though the report itself is read-only.

**Root cause:** the team treated a read-only transaction as harmless. Its long-lived transaction-level snapshot kept an old visibility horizon relevant while write-heavy tables continued producing obsolete row versions. The transaction boundary included remote waiting that did not need database snapshot consistency.

**Correct pattern:** keep `Repeatable Read` reporting transactions as short as the consistency window actually requires, do not hold database transactions open across unrelated remote I/O, monitor long-running transactions and `idle in transaction` sessions, set appropriate transaction/session timeouts, and observe dead tuples plus vacuum progress instead of reacting only after disk growth becomes obvious.
...
Check long-running snapshots/transactions, replication or maintenance horizons where relevant, dead tuple accumulation, vacuum progress, and transaction age.
...
- [ ] **Remote work:** avoid holding a retained transaction snapshot open across unrelated network calls, user think time, or queue waits.
- [ ] **Vacuum health:** monitor dead tuples, vacuum/autovacuum activity, relation growth, and maintenance lag on write-heavy tables.
- [ ] **Idle sessions:** detect and bound `idle in transaction` sessions, especially when an isolation level or cursor retains an old snapshot.
