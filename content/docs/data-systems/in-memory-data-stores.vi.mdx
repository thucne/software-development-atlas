---
title: "In-Memory Data Stores: Suy luận về độ trễ, bộ nhớ và độ bền dữ liệu"
...
# In-Memory Data Stores: Suy luận về độ trễ, bộ nhớ và độ bền dữ liệu

## Tóm tắt

Đúng 20:00 giờ cao điểm, độ trễ API toàn hệ thống bất ngờ vọt từ 5ms lên 35.000ms rồi trả về lỗi 500 hàng loạt. Báo cáo phân tích nguyên nhân gốc rễ (post-mortem) chỉ ra chuỗi thảm họa dây chuyền: đầu tiên, một kỹ sư tiện tay gõ lệnh `KEYS *` trên production để điều tra lỗi, lập tức khóa chặt luồng xử lý đơn (single thread) của Redis suốt 10 giây. Cùng thời điểm đó, vì hàng triệu bản ghi cache không hề được gắn thời gian sống (**TTL**) và `maxmemory-policy` cấu hình thiếu ranh giới giải phóng bộ nhớ, RAM chạm ngưỡng 100% khiến hệ điều hành kích hoạt OOM killer tiêu diệt ngay tiến trình Redis. Khi node được khởi động lại nguội ngắt, một khóa truy cập cực lớn (**hot key**) của trang chủ vừa hết hạn, tạo ra cơn bão **cache stampede (thundering herd)** với 40.000 request ập thẳng xuống cơ sở dữ liệu quan hệ, đánh sập luôn **nguồn sự thật (source of truth)** chính của doanh nghiệp.

> 💡 **Quy tắc bỏ túi:** RAM là một ngân sách dung lượng hữu hạn và đắt đỏ, không phải chiếc đũa thần tốc độ vô tận. Hãy phân định rạch ròi: in-memory store là lớp **cache** có thể giải phóng bất cứ lúc nào (dựa vào **nguồn sự thật** phía sau), hay là nguồn lưu trữ chính có cam kết **bền vững (persistence)** và **sao chép (replication)**—tuyệt đối không trộn lẫn hai vai trò này trong cùng một bộ nhớ chịu áp lực giải phóng (eviction).

- **RAM quyết định vị trí đặt, không quyết định độ bền:** In-memory data store giữ **tập dữ liệu nóng (working set)** trong **RAM** để đạt độ trễ cực thấp, nhưng độ bền vững đòi hỏi chiến lược lưu trữ bền vững (**persistence**) qua snapshot **RDB** và ghi log **AOF**, song song với cơ chế **replication** bất đồng bộ hoặc đồng bộ.
- **Eviction là cam kết về tính đúng đắn:** Khi chạm ngưỡng **maxmemory**, chính sách giải phóng bộ nhớ (**eviction**) sẽ loại bỏ key; nếu lưu token xác thực, bộ đếm giới hạn tốc độ hoặc khóa bất biến (idempotency key) chung với **cache** thông thường, hệ thống sẽ gánh chịu lỗi sai lệch logic nghiêm trọng khi bị evict sớm.
- **TTL quản lý vòng đời, không thay thế cho quản lý dung lượng:** **TTL** ấn định thời điểm bản ghi hết hạn; kết hợp TTL với độ lệch ngẫu nhiên (jitter) giúp triệt tiêu các đợt hết hạn đồng loạt và ngăn chặn thảm họa **cache stampede** (hiệu ứng dồn dập / thundering herd).
- **Sharding mở rộng thông lượng, không cứu được hot key:** Redis Cluster phân chia dữ liệu qua 16.384 **hash slot** trên nhiều **shard**, nhưng một **hot key** có lưu lượng đột biến vẫn dồn 100% tải vào đúng một shard duy nhất.
- **Cạm bẫy chết người (Khóa đơn luồng & sập nguồn OOM):** Lưu trữ dữ liệu vô tội vạ không cài đặt **TTL**, dùng sai `maxmemory-policy` dẫn đến crash OOM vào giờ cao điểm, hoặc vô tình chạy các lệnh duyệt quét chặn luồng như `KEYS *` làm nghẽn toàn bộ luồng xử lý đơn của Redis.

Mental model:

```text
yêu cầu gửi đến
  -> chọn khóa + cấu trúc dữ liệu phù hợp
  -> truy cập tập dữ liệu nóng nằm sẵn trên RAM
  -> có thể hết hạn hoặc bị giải phóng theo chính sách
  -> có thể ghi bền vững xuống ổ đĩa lưu trữ
  -> có thể sao chép sang node khác
  -> có thể định tuyến / phân mảnh qua cụm cluster
```

Mỗi chữ “có thể” ở trên là một quyết định về cam kết dịch vụ (contract decision), không phải chi tiết triển khai phụ.

```mermaid
flowchart LR
  C[Thao tác từ client] --> K[Key + cấu trúc dữ liệu]
  K --> M[Dữ liệu thường trú trên RAM]
  M --> E{Áp lực bộ nhớ?}
  E -->|có| V[Chính sách eviction]
  M --> T[TTL / hết hạn]
  M --> P[Chính sách persistence]
  M --> R[Chính sách replication]
  M --> S[Vị trí đặt shard / key]
```

<TermBox term="Working set">
**Working set (Tập dữ liệu nóng)** là phần dữ liệu mà khối lượng công việc (workload) cần truy cập liên tục trong một khoảng thời gian xác định.

**Vì sao quan trọng:** Thiết kế in-memory chỉ đạt hiệu quả kinh tế khi tập dữ liệu nóng nằm vừa vặn trong ngân sách bộ nhớ (RAM). Nếu ứng dụng liên tục chạm vào lượng dữ liệu lớn hơn dung lượng RAM sẵn có, hiện tượng giải phóng bộ nhớ (eviction), trượt cache (cache miss) và đọc dữ liệu từ xa sẽ chi phối toàn bộ độ trễ hệ thống.
</TermBox>
...
## 4. Eviction là cam kết về tính đúng đắn của hệ thống

<TermBox term="Eviction">
**Eviction (Giải phóng bộ nhớ cưỡng bức)** là thao tác máy chủ chủ động loại bỏ bớt các bản ghi để giữ mức tiêu thụ bộ nhớ không vượt quá giới hạn cho phép (`maxmemory`).

Redis hỗ trợ nhiều chính sách như `allkeys-lru`, `allkeys-lfu`, các biến thể chỉ áp dụng trên key có gắn thời hạn (`volatile-*`), chọn ngẫu nhiên hoặc `noeviction` (từ chối mọi lệnh ghi mới khi bộ nhớ đầy).

**Vì sao quan trọng:** Một khi đã bật eviction, tầng ứng dụng phải chấp nhận một tiền đề: bất kỳ bản ghi nào đủ điều kiện đều có thể bốc hơi trước khi code chủ động xóa. Nếu dữ liệu đó mang tính sống còn đối với logic nghiệp vụ, việc bị evict sớm sẽ gây sai lệch trạng thái nghiêm trọng.
</TermBox>

Với một **cache** thuần túy (disposable cache), eviction là hành vi hoàn toàn bình thường: trượt cache thì chỉ việc tải lại từ **nguồn sự thật (source of truth)**.

Nhưng với trạng thái ảnh hưởng đến tính đúng đắn (correctness-bearing state), eviction có thể trở thành thảm họa:

```text
idempotency record bị evict sớm -> yêu cầu gửi lại có thể bị thực thi lần 2 (trùng lặp thanh toán)
bộ đếm rate-limit bị evict       -> client có thể spam vượt quá hạn mức cho phép
session state bị evict            -> người dùng bị văng phiên đăng nhập giữa chừng
lease metadata bị evict           -> các tiến trình phân tán tranh chấp tài nguyên lẫn nhau
```

Tuyệt đối không đặt các trạng thái quan trọng này vào chung một cụm bộ nhớ có bật eviction với các dữ liệu cache thông thường.

### LRU và LFU là thuật toán gần đúng, không thấu hiểu giá trị nghiệp vụ

Redis tài liệu hóa rõ ràng rằng các giải thuật LRU và LFU trong bộ nhớ là các phép xấp xỉ tối ưu hóa hiệu năng, chứ Redis không thể tự hiểu giá trị nghiệp vụ của từng bản ghi.

Một bản ghi idempotency ít khi được đọc lại có thể quan trọng gấp trăm lần một bản ghi cache thông tin sản phẩm được truy vấn hàng nghìn lần mỗi giây. Tần suất truy cập đơn thuần không thể phản ánh được mức độ quan trọng sống còn của dữ liệu.

Hãy tách biệt các khối lượng công việc (workload) khi ý nghĩa nghiệp vụ và khả năng chấp nhận rủi ro mất mát khác nhau.
...
Ưu điểm là snapshot compact và bulk restore nhanh. Đánh đổi là write sau snapshot gần nhất có thể mất nếu process/host chết trước snapshot tiếp theo.
...
- replicate read-mostly hot data nếu consistency đánh đổi cho phép;
...
Điều này tăng locality nhưng cũng tập trung các key đó lên một shard. Locality và distribution là đánh đổi, không phải hai knob độc lập.
...
Bài này được phân loại **evolving** với chu kỳ review 180 ngày vì eviction option, persistence behavior, clustering capability và operational guidance của Redis tiếp tục thay đổi dù các đánh đổi cốt lõi về memory/durability khá bền vững.
