...
Trong một sự cố sập hệ thống thanh toán giờ cao điểm, năm kỹ sư ngồi bất lực trước bức tường 40 biểu đồ Grafana không có điểm chung trong khi khách hàng liên tục bị lỗi mà không thể thanh toán. Hệ thống log là một vòi rồng hỗn loạn xả ra 200.000 dòng văn bản thô không chỉ mục mỗi giây; chỉ số metric báo lỗi 5xx tăng vọt nhưng không thể chỉ ra lỗi xuất phát từ tenant hay service nào; còn distributed tracing thì bị bỏ qua. Cả đội mất hơn 2 tiếng đồng hồ đoán mò trên kênh chat thay vì chẩn đoán dựa trên chứng cứ tương quan.

> 💡 **Quy tắc bỏ túi:** **Metric phát hiện triệu chứng, Trace cô lập nguyên nhân, và Log phơi bày sự thật cục bộ.** Dữ liệu giám sát sẽ vô dụng nếu thiếu sự tương quan: chỉ số metric kích hoạt cảnh báo, mã `trace_id` định vị chính xác chặng mạng hoặc truy vấn bị nghẽn, và log có cấu trúc giải thích tường tận quyết định nội bộ của service tại đúng thời điểm đó.

- **Bộ ba tín hiệu bổ trợ:** Metric theo dõi sức khỏe số lượng tổng thể (phương pháp RED); distributed trace dựng lại quan hệ nhân quả của request xuyên qua các ranh giới dịch vụ; structured log ghi lại chi tiết bối cảnh sự kiện.
- **Tương quan qua ngữ cảnh dùng chung:** Đồng bộ hóa `trace_id` và `span_id` xuyên suốt ranh giới tiến trình, thư viện ghi log và ngữ cảnh OpenTelemetry.
- **Kiểm soát độ biến thiên (Cardinality):** Tuyệt đối không đưa các giá trị biến thiên cao (`user_id`, UUID, email) vào nhãn metric để tránh làm phình to bộ nhớ cơ sở dữ liệu chuỗi thời gian và bùng nổ chi phí.
- **Kinh tế học lấy mẫu (Sampling):** Lấy mẫu head-sampling tỷ lệ thấp (1–5%) cho các request thành công thông thường, và lấy mẫu tail-sampling 100% cho mọi request lỗi hoặc có độ trễ đuôi (tail latency) cao.
- **Cạm bẫy chết người:** Ghi log dạng chuỗi thô (`console.log("lỗi: " + err)`) không có trường JSON chuẩn hóa hoặc thiếu correlation ID—khiến việc tìm kiếm log và cảnh báo tự động hoàn toàn tê liệt khi xảy ra sự cố production.
...
**Tại sao quan trọng:** dashboard chỉ xác nhận các lỗi đã biết là monitoring. Một hệ observability hữu ích còn cho phép kỹ sư đặt câu hỏi mới trong lúc sự cố đang diễn ra.
</TermBox>
...
API của bạn expose `request_duration_seconds{user_id, route, status}` và emit logs không có trace context. Trong sự cố, bạn thấy error-rate spike nhưng không tìm được log cho các request thuộc slow trace.
