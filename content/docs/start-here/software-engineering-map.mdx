...
description: See how Atlas domains, concepts, content, and future learning paths fit together.
category: start-here
level: beginner
status: evergreen
...
The map is intentionally broad before it is complete. A concept is allowed to exist before a dedicated lesson does; that makes missing coverage visible instead of letting the current sidebar define the perceived shape of software engineering.

## Map, content, path, and decision guide
...
**Learning path** answers: *In what sequence should I learn a useful slice of the map?* Curated learning paths are a later Atlas capability; the map does not imply one universal reading order.
