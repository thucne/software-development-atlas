...
> *Mô tả hình minh họa:* Sơ đồ tổng thể kiến trúc Event Loop của trình duyệt gồm 4 khối chính:
> 1. **Call Stack:** Thực thi mã JavaScript đồng bộ theo cơ chế LIFO (Run-to-completion).
> 2. **Web APIs:** Các tác vụ chạy ngầm của trình duyệt (Timers, Fetch network, DOM Event listeners).
> 3. **Microtask Queue:** Hàng đợi ưu tiên cao (`Promise.then`, `queueMicrotask`, `MutationObserver`), được xả sạch tại mỗi checkpoint.
> 4. **Task Queues (Macrotasks):** Các hàng đợi công việc định kỳ/sự kiện (`setTimeout`, I/O, Click handlers).
> Kèm mũi tên điều phối của Event Loop Engine quay vòng kiểm tra Call Stack và các hàng đợi.
...
> *Mô tả hình minh họa:* Sơ đồ vòng lặp xả cạn Microtask Queue: Khi task chính hoàn tất -> Bước vào Microtask Checkpoint -> Lấy từng microtask ra thực thi -> Nếu microtask đó đẻ thêm microtask mới qua `queueMicrotask()` -> Tiếp tục đẩy vào cuối hàng đợi hiện tại và chạy tiếp trong CÙNG checkpoint cho đến khi queue hoàn toàn rỗng (`empty`) -> Lúc đó trình duyệt mới chuyển sang xem xét Rendering hoặc Task tiếp theo.
...
> *Mô tả hình minh họa:* So sánh trực quan 2 cơ chế làm nghẽn Main Thread (Long Task vs Microtask Starvation):
