...
description: "Hiểu sâu sắc về task queue, microtask, checkpoint, rendering pipeline, đói tài nguyên và sự khác biệt căn bản so với Node.js."
...
4. **Tránh nghẽn Main Thread do đói tài nguyên (Microtask Starvation):** Vòng lặp đệ quy `queueMicrotask()` sẽ làm "đóng băng" giao diện trình duyệt hoàn toàn dù không có vòng lặp `while(true)` đồng bộ nào.
