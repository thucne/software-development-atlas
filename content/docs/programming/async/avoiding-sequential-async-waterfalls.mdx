...
> *Mô tả hình minh họa:* So sánh trực quan tab Network trong DevTools giữa 2 luồng:
> - **Bậc thang nối tiếp (Waterfall):** Request A (800ms) -> Request B (400ms) -> Request C (300ms) xếp thành bậc thang chéo tổng cộng 1500ms.
> - **Song song (Overlapped):** Request A, B, C cùng xuất phát tại t = 0ms, kết thúc khi request A dài nhất xong (800ms).
...
> *Mô tả hình minh họa:* Sơ đồ đồ thị DAG thể hiện luồng thực thi:
> - Nhánh phụ thuộc: `getUser(id)` (500ms) -> `getOrganization(user.orgId)` (400ms) tạo thành Critical Path (900ms).
> - Các nhánh độc lập: `getFeatureFlags(id)` (300ms) và `getRecommendations(id)` (700ms) được kích hoạt ngay từ t = 0 và hoàn thành trước khi Critical Path kết thúc.
...
> *Mô tả hình minh họa:* Sơ đồ cơ chế kiểm soát fan-out: Một mảng 1,000 requests độc lập được điều phối qua một Semaphore/Worker Pool với `concurrency = 5`. Tại mỗi thời điểm chỉ có tối đa 5 kết nối đang mở tới database, các tác vụ còn lại nằm trong hàng đợi chờ slot trống, giữ cho latency ổn định mà không làm sập downstream.
