...
`p0` fulfills with `2`; `p1` fulfills with `6`; `p2` rejects with `Error('boom')`. When the `catch()` handler returns still-pending `inner`, `p3` is pending but resolved/adopting `inner`. After `inner` fulfills with `10`, `p3` fulfills with `10` and `p4` fulfills with `11`.
