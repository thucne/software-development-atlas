...
For browser-only JavaScript, Keycloak's documentation says the client must be public because there is no secure way to store client credentials in client-side code.
