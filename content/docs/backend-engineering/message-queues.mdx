...
> 💡 **Rule of thumb:** Transport delivery completion is not business completion. Never acknowledge (ACK) a message before its durable effect is committed, protect consumers with Dead-Letter Queues (DLQ) and exponential backoff against poison pills, and make handlers idempotent to absorb inevitable redeliveries.
...
sequenceDiagram
  ...
  Q->>W1: deliver M (becomes in-flight)
