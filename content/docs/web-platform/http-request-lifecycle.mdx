...
> *Illustration description:* Branching diagram comparing 4 real-world request journeys:
> 1. **Path 1 (Browser Local Cache Hit):** Client checks local storage/cache -> Response is `fresh` -> Returns data immediately (0ms network latency; skips DNS, TCP, TLS, and backend entirely).
> 2. **Path 2 (Connection Reused):** Local cache miss -> Client finds an open HTTP/2 or HTTP/3 connection to the target origin -> Dispatches HTTP stream over the existing socket (skips DNS resolution, TCP 3-way handshake, and TLS negotiation).
> 3. **Path 3 (Full Cold Setup):** No suitable connection exists -> Executes full setup chain: DNS Query -> TCP SYN/ACK (1 RTT) -> TLS 1.3 Key Exchange (1 RTT) -> HTTP Request/Response (1 RTT).
> 4. **Path 4 (CDN Intermediary Edge Hit):** Request leaves client across the network but hits nearest CDN edge POP -> CDN serves cached response -> Saves hundreds of milliseconds and prevents origin compute load.
...
> *Illustration description:* Browser cache decision tree:
> - Request initiated -> Check Local Cache:
>   - **Cache Miss:** -> Dispatches full network request to the internet.
>   - **Cache Hit:**
>     - If still within `max-age` (Fresh): Serves stored response immediately -> `200 OK (from disk cache)`.
>     - If expired (Stale):
>       - Checks whether stored response has validator tokens (`ETag` or `Last-Modified`)?
>       - If YES: Dispatches **Conditional Request** with `If-None-Match: "xyz"` or `If-Modified-Since`.
>         - Server determines representation is unchanged -> Responds with `304 Not Modified` (empty body, headers only) -> Browser reads body from local cache.
>         - Server determines representation changed -> Responds with `200 OK` + full replacement body -> Browser updates cache.
...
> *Illustration description:* Visual comparison of network transport mechanisms across three HTTP generations:
> 1. **HTTP/1.1 (Head-of-Line Blocking over TCP):** Each request must wait for the preceding response to fully transfer on a single TCP connection. To download concurrent resources, browsers open up to 6 parallel TCP connections, consuming server socket resources.
> 2. **HTTP/2 (Multiplexing over a Single TCP Connection):** Splits requests/responses into interleaved binary frames (Data and Header frames) across logical streams over one TCP pipe. Eliminates application-layer HoL blocking, but a single lost TCP packet stalls all multiplexed streams awaiting TCP retransmission (TCP HoL Blocking).
> 3. **HTTP/3 (QUIC over UDP):** Employs QUIC transport over UDP. Streams are completely independent at the transport layer; dropping a packet in image stream A does not delay delivery of data on API streams B and C.
...
> *Illustration description:* Multi-hop path from client to origin application:
> `Client (Browser/App)` ➔ `Edge CDN (e.g. Cloudflare/Fastly - Edge Caching, DDoS Protection)` ➔ `API Gateway / Reverse Proxy (e.g. Nginx/Envoy - TLS Termination, Auth, Rate Limiting)` ➔ `Internal Load Balancer` ➔ `Origin Application Service (Node.js/Go/Java)`.
> - Visual callout isolating response boundary origins:
>   - Errors `403/429` generated by CDN edge WAF.
>   - Errors `502 Bad Gateway / 504 Gateway Timeout` generated by the API Gateway unable to reach backends.
>   - Errors `500 Internal Server Error` or application `200 OK` produced by origin application code.
> - Directs developers to inspect the correct logs (CDN logs vs Gateway logs vs Application traces) rather than guessing.
