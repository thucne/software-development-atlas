...
> *Illustration description:* Detailed timeline comparison diagram:
> - **Cold Start (100ms – multiple seconds):** Sandbox resource allocation -> Pull image/code bundle -> Initialize language runtime -> Execute top-level initialization code (DB connection, config load) -> Process request.
> - **Warm Start (a few ms):** Reuse existing warm execution environment -> Receive and process request immediately.
...
> *Illustration description:* Two-axis matrix: Horizontal axis represents "Packaging Model" (Application Code / Zip vs OCI Container Image); Vertical axis represents "Operating Model" (Self-managed VMs vs Managed Container Cluster vs Serverless Managed Execution Environment). Highlights where products intersect (e.g., Cloud Run = Container Packaging + Serverless Operating).
...
> *Illustration description:* Traffic surge diagram: 1,500 serverless functions spin up concurrently -> each function establishes 1 TCP connection to the database -> exceeds the database connection limit (150 connections) -> database starves and emits Connection Refused / Timeout errors across the entire fleet.
...
> *Illustration description:* Cost curve chart over traffic volume: X-axis represents "Request Volume", Y-axis represents "Monthly Cost ($)". Serverless curve starts at $0 (scale to zero) and slopes steeply upward with request count; Container/VM curve has a higher initial baseline cost but flattens out under sustained high traffic. The intersection (Crossover Point) marks the economic transition point between models.
