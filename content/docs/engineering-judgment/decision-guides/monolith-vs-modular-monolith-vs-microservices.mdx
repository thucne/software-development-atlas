...
> *Illustration description:* Visual comparison of three architectural topologies: Monolith (all modules co-located in 1 process & 1 DB), Modular Monolith (modules enforce strict internal boundaries within 1 deployment & shared DB transaction boundary), and Microservices (autonomous services communicating over network/events with independent databases).
...
> *Illustration description:* Visual comparison of two incident blast-radius scenarios:
> - **Monolith Failure:** A memory leak bug in a non-critical Reporting module causes an Out-Of-Memory (OOM) process crash -> freezes the entire application, knocking out customer Auth and Checkout flows.
> - **Isolated Microservices with Circuit Breaker:** The Reporting service experiences a severe outage, but the API Gateway trips its circuit breaker and serves a fallback -> core Checkout and Auth maintain 100% SLA uptime.
...
> *Illustration description:* Comparison contrasting an in-process local call (single atomic ACID DB commit/rollback) against a distributed network call (Service A successfully commits -> Service B experiences network timeout -> requires compensating actions / Saga workflow to restore data consistency).
...
> *Illustration description:* Diagram of a "Distributed Monolith" anti-pattern: 6 microservices packaged as distinct deployables, but locked in synchronous HTTP daisy chains (Service A calls B -> B calls C -> C calls D), sharing a single database and requiring coordinated lockstep releases. Impact: compounding latency, cascading failure multiplier, loss of deployment autonomy, and 5x higher operational overhead.
