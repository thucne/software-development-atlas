...
> *Mô tả hình minh họa:* Sơ đồ 3 dòng timeline so sánh thời điểm: Browser nhận HTML (rỗng vs đầy đủ), Tải JS bundle, Hydration hoàn tất, và thời điểm User tương tác được (First Contentful Paint vs Time to Interactive / INP).
...
> *Mô tả hình minh họa:* Sơ đồ 3 giai đoạn của trình duyệt khi load trang SSR:
> 1. Nhận Server-rendered HTML: Giao diện hiển thị đầy đủ ngay lập tức, user thấy nội dung nhưng DOM chưa gắn event listener.
> 2. Giai đoạn "Uncanny Valley": User nhấp chuột vào button/menu nhưng không phản hồi do JavaScript bundle vẫn đang tải/parse.
> 3. Hydration hoàn tất: Framework gắn event listeners vào các node DOM tĩnh, trang đạt trạng thái tương tác hoàn toàn (Time to Interactive).
...
> *Mô tả hình minh họa:* Sơ đồ chu trình Revalidation tại Edge CDN:
> - Request 1 (Trong hạn fresh): Trả về ngay lập tức từ Edge cache (0ms server compute).
> - Request 2 (Quá hạn stale window): Vẫn trả về nội dung cũ ngay tức thì cho user (Stale-while-revalidate), đồng thời ngầm kích hoạt Server sinh bản render mới (Background revalidation).
> - Request 3 (Sau khi sinh xong): CDN cập nhật bản mới và trả về cho toàn bộ người dùng tiếp theo.
...
> *Mô tả hình minh họa:* Sơ đồ phân rã một trang thương mại điện tử thực tế thành các tầng rendering: Header/Footer tĩnh (SSG qua CDN), Product Detail render từ server (SSR), và Giỏ hàng / Đề xuất cá nhân hóa tải bất đồng bộ phía client (CSR).
