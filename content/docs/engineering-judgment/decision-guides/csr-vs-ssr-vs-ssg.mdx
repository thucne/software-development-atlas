...
> 🖼️ **[Illustration Placeholder: Timeline comparison of CSR vs SSR vs SSG]**  
> *Illustration description:* Three-lane timeline comparison diagram showing milestones: Browser receives HTML (empty shell vs full DOM), JavaScript bundle download/parse, Hydration completes, and User interaction capability (First Contentful Paint vs Time to Interactive / INP).
...
> 🖼️ **[Illustration Placeholder: Hydration Process & The "Uncanny Valley" Gap]**  
> *Illustration description:* Three-phase browser diagram for SSR page loads:
> 1. Receive Server-rendered HTML: Visual interface renders immediately; user sees content but DOM nodes lack active event listeners.
> 2. The "Uncanny Valley": User clicks buttons or opens menus, but the UI remains unresponsive while the JavaScript bundle downloads, parses, and executes.
> 3. Hydration Complete: Framework attaches event listeners to static DOM nodes; the page reaches full interactive capability (Time to Interactive).
...
> 🖼️ **[Illustration Placeholder: Incremental Static Revalidation (ISR) Life Cycle]**  
> *Illustration description:* Revalidation cycle at an Edge CDN:
> - Request 1 (Fresh cache hit): Served instantly from Edge cache (0ms origin compute).
> - Request 2 (Stale window reached): Immediately serves stale content to user (stale-while-revalidate), while asynchronously triggering origin server generation (background revalidation).
> - Request 3 (After generation completes): Edge cache updates with the regenerated page and serves it to all subsequent requests.
...
> 🖼️ **[Illustration Placeholder: Hybrid Page Architecture & Island / Streaming Flow]**  
> *Illustration description:* Architectural decomposition of a modern e-commerce page across rendering tiers: static global header/footer (SSG via CDN), core product details server-rendered (SSR), and dynamic shopping cart / personalized recommendations fetched asynchronously on the client (CSR).
