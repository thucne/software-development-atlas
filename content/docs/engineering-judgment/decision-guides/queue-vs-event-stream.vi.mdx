---
title: "Message Queue hay Event Stream: Cách chọn đúng công nghệ nhắn tin"
...
Hãy tưởng tượng bảng ghim order trong một gian bếp nhà hàng: một phiếu order "Bàn số 4: Bò bít tết vừa chín tới" được ghim lên thanh ray. Người đầu bếp nào rảnh tay đầu tiên sẽ giật phiếu xuống, chế biến món ăn và vứt phiếu vào thùng rác—nhiệm vụ kết thúc và biến mất vĩnh viễn. Đó chính là **Hàng đợi công việc (Worker Queue)**. Giờ hãy tưởng tượng cuốn sổ cái kế toán của doanh nghiệp: từng giao dịch thu chi được ghi chép bất biến theo thứ tự thời gian vào cuốn nhật ký nối tiếp. Kiểm toán viên, kế toán trưởng và hệ thống phân tích thuế đều có thể độc lập lật từng trang sổ cái theo tốc độ của riêng mình, đọc đi đọc lại qua nhiều năm mà không làm mất đi một dòng nhật ký nào. Đó chính là **Luồng sự kiện phân vùng (Partitioned Log Stream)**. Thảm họa sản xuất nổ ra khi kỹ sư nhầm lẫn hai mô hình này: đưa sự kiện nghiệp vụ cốt lõi vào hàng đợi cạnh tranh khiến các service tranh nhau cướp mất dữ liệu của nhau, hoặc vác Kafka cồng kềnh về chỉ để làm tác vụ resize ảnh đại diện rồi chìm trong mớ bòng bong rebalance và cạn kiệt ổ cứng.

> 💡 **Quy tắc bỏ túi:** Hãy xác định bản ghi là một nhiệm vụ (Job) cần một người hoàn thành rồi xóa, hay là một sự thật (Fact) mà nhiều hệ thống độc lập cần cùng theo dõi. Dùng Hàng đợi công việc (như RabbitMQ, Amazon SQS) khi cần chia việc cho các worker tranh nhau xử lý với cơ chế xác nhận (Ack) và thử lại từng tin nhắn qua hàng đợi thư chết (Dead-letter queue); dùng Luồng sự kiện (như Apache Kafka, Redpanda) khi nhiều nhóm consumer độc lập cần đọc, duy trì thứ tự và phát lại (Replay) chuỗi sự kiện bất biến theo thời gian.

- **Hàng đợi công việc chuyên chia nhỏ đơn vị công việc:** Nhiều worker cùng tranh nhau xử lý một hàng đợi; khi worker xác nhận (ACK) thành công, tin nhắn sẽ bị xóa. Lựa chọn tối ưu cho tác vụ nền, xử lý hàng loạt và gửi email/gọi webhook.
- **Luồng sự kiện lưu giữ nhật ký sự thật bất biến:** Dữ liệu được ghi nối tiếp vào log và lưu giữ theo thời gian. Các nhóm consumer độc lập tự quản lý vị trí đọc (offset) và có thể phát lại lịch sử tùy ý mà không làm ảnh hưởng đến bên khác.
- **Thứ tự được đảm bảo theo phân vùng, không phải toàn cục:** Luồng sự kiện chỉ đảm bảo thứ tự FIFO tuyệt đối bên trong cùng một khóa phân vùng (partition key); hàng đợi có nhiều worker song song sẽ hoàn thành công việc không theo thứ tự đưa vào.
- **Ngữ nghĩa xác nhận và rủi ro trùng lặp:** Hàng đợi tự động gửi lại tin nhắn chưa được xác nhận; luồng sự kiện commit vị trí đọc sau khi xử lý. Cả hai đều không tự động đem lại "chính xác một lần" (exactly-once)—bên nhận bắt buộc phải có cơ chế **idempotency** (tính lặp an toàn).
- **Cạm bẫy chết người:** **Dùng nhầm một hàng đợi công việc duy nhất để phát tán sự kiện (Fan-out).** Gắn cả service gửi mail, kế toán và phân tích dữ liệu vào chung một hàng đợi cạnh tranh sẽ khiến mỗi sự kiện chỉ rơi ngẫu nhiên vào một service duy nhất, làm mất trắng dữ liệu ở các service còn lại. Bắt buộc phải tách hàng đợi riêng cho từng subscriber hoặc xuất bản vào một luồng sự kiện phân vùng.
