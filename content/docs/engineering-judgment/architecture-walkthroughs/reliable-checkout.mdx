...
> *Illustration description:* Sequence diagram explicitly delineating the 5 operational boundaries: (1) Client -> API (with Idempotency-Key), (2) API -> DB (Local ACID Transaction wrapping Order + Outbox row), (3) API -> Payment Provider (Timeout Ambiguity window), (4) Outbox Publisher -> Message Broker (At-least-once handoff), (5) Broker -> Fulfillment Consumer (Idempotent execution).
...
> *Illustration description:* Ambiguity window diagram when a client issues a payment request: if the response packet is lost in transit, the client cannot distinguish whether the third-party provider debited the funds or aborted without a reconciliation protocol or idempotent query.
...
> *Illustration description:* Architectural comparison:
> - **Dual-Write Failure:** App writes to database successfully -> App calls Message Broker but crashes or hits network drop -> State in DB is committed but no downstream event is published (silent persistent inconsistency).
> - **Transactional Outbox Solution:** App writes Order and Outbox Event within the same local ACID database transaction -> Background worker polls/streams the Outbox table and publishes to Broker -> Guarantees at-least-once delivery even across process crashes.
...
> *Illustration description:* Two-wave traffic graph:
> - **Retry Storm:** When a downstream payment gateway recovers from a 10s outage, thousands of clients retry synchronously at fixed intervals (t = 1s, 2s, 4s), producing sharp synchronized traffic spikes that immediately crash the gateway again.
> - **Full Jitter:** Clients distribute retry attempts randomly across `[0, Backoff(n)]`, producing a smoothed, flat request curve that allows downstream dependencies to recover safely.
