...
> 🖼️ **[Illustration Placeholder: End-to-end Checkout Sequence & Consistency Boundaries]**  
> *Mô tả hình minh họa:* Sơ đồ Sequence Diagram thể hiện rõ 5 ranh giới: (1) Client -> API (với Idempotency-Key), (2) API -> DB (Local ACID Transaction bọc Order + Outbox row), (3) API -> Payment Provider (Timeout Ambiguity window), (4) Outbox Publisher -> Message Broker (At-least-once handoff), (5) Broker -> Fulfillment Consumer (Idempotent execution).
...
> 🖼️ **[Illustration Placeholder: Two Generals Problem & Payment Ambiguity Window]**  
> *Mô tả hình minh họa:* Sơ đồ minh họa "vùng bất định" khi client gửi request thanh toán: nếu gói tin response bị mất trên đường truyền mạng, client không thể biết server đối tác đã trừ tiền hay chưa nếu không có cơ chế Reconciliation hoặc Idempotent Query.
...
> 🖼️ **[Illustration Placeholder: Dual-Write Problem vs Transactional Outbox Pattern]**  
> *Mô tả hình minh họa:* Sơ đồ đối chiếu:
> - **Dual-Write Failure:** App ghi vào DB thành công -> App gọi Message Broker bị crash/lỗi mạng -> State trong DB đã cập nhật nhưng không có event nào được bắn đi (Dữ liệu bị lệch vĩnh viễn).
> - **Transactional Outbox Solution:** App ghi Order và Outbox Event trong cùng 1 local DB transaction -> Worker đọc Outbox table và publish sang Broker -> Đảm bảo At-least-once delivery ngay cả khi process crash.
...
> 🖼️ **[Illustration Placeholder: Thundering Herd Retry Storm vs Exponential Backoff with Jitter]**  
> *Mô tả hình minh họa:* Đồ thị 2 đợt sóng traffic:
> - **Retry Storm:** Khi payment gateway phục hồi sau sự cố 10s, hàng nghìn client gửi retry đồng loạt tại đúng các mốc t = 1s, 2s, 4s tạo thành các đỉnh nhọn xung kích (Traffic Spikes) tiếp tục đánh sập gateway lần nữa.
> - **Full Jitter:** Các client phân tán thời gian retry ngẫu nhiên trong khoảng `[0, Backoff(n)]`, tạo ra một đường tải phẳng mượt mà cho phép downstream phục hồi an toàn.
