...
test.describe('Announcement banner, changelog, and status badges', () => {
  test('renders top release announcement banner on docs pages', async ({ page }) => {
    ...
    const banner = page.locator('#atlas-release-2026-09-22');
    await expect(banner).toBeVisible();
    await expect(banner.getByText('New', { exact: true })).toBeVisible();
    await expect(
      banner.getByText('54 new lessons added since Sep 10!', { exact: true }),
    ).toBeVisible();
    await expect(
      banner.getByText(
  ...
  for (const width of [390, 360]) {
    test(`keeps the New badge visible and shortens copy on ${width}px mobile viewport`, async ({
      ...
      const primaryCopy = banner.getByText('54 new lessons added since Sep 10!', {
        exact: true,
      });
  ...
  test('renders Vietnamese localized banner on /vi/docs pages', async ({ page }) => {
    ...
    await expect(
      banner.getByText('54 bài học mới được bổ sung từ 10/09!', { exact: true }),
    ).toBeVisible();
    await expect(
