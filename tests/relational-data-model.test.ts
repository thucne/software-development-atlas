...
describe('Relational Data Model lesson', () => {
  ...
  it('keeps the bilingual September release ledger and banner current', () => {
    for (const relativePath of [
      ...
      expect(source, relativePath).toContain('20');
    ...
    expect(banner).toContain('20 bài học kiến trúc hệ thống');
    expect(banner).toContain('20 new system architecture lessons');
  });
});
